...
namespace TicketSalesSystem.ReactUI.Controllers
{
	...
	public class FlightController : ControllerBase
	{
		...
		[HttpPost]
		[Route("searchChipest")]
		public async Task<ActionResult<IEnumerable<FlightDTO>>> GetCheapestFlights([FromBody] FlightSearchCriteria criteria)
		{
			var flights = await _flightService.GetAllAsync();
			var res = flights.Where(f => f.Route.DeparturePoint.ToLower() == criteria.DeparturePoint.ToLower() && f.Route.ArrivalPoint.ToLower() == criteria.ArrivalPoint.ToLower())
				.OrderBy(f => f.Price)
				.FirstOrDefault();
			return Ok(res);
		}
